...
const state = { elapsed: 0, lastTick: performance.now(), lastDisplaySecond: -1, running: true, speed: 1, selectedId: BOATS[0].id, boats: new Map(), markers: new Map(), routeLayers: new Map(), activity: [], rentalStarts: new Set(), manualRentals: new Map(), pendingBoatId: null };
const elements = { fleetList: document.getElementById("fleetList"), rentalCount: document.getElementById("rentalCount"), activityList: document.getElementById("activityList"), simulationTime: document.getElementById("simulationTime"), selectedBoat: document.getElementById("selectedBoat"), toggle: document.getElementById("toggleSimulation"), reset: document.getElementById("resetSimulation"), speed: document.getElementById("speedControl"), speedValue: document.getElementById("speedValue"), rentalModal: document.getElementById("rentalModal"), rentalBackdrop: document.getElementById("rentalBackdrop"), rentalForm: document.getElementById("rentalForm"), rentalConfirm: document.getElementById("rentalConfirm") };
...
function updateBoatStates() {
    BOATS.forEach(boat => {
        ...
        const next = { ...boat, phase, guest, location: boatLocation(boat, phase) };
        ...
        if (phase === "checkin" && prior && prior.phase !== "checkin") addActivity(`${boat.name} returned; guest details cleared.`);
...
function resetSimulation() { state.elapsed = 0; state.activity = []; state.rentalStarts.clear(); state.manualRentals.clear(); state.boats.clear(); updateBoatStates(); render(); }
function tick(now) {
