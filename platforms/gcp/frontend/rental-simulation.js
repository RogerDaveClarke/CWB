...
function renderFleet() {
    ...
    elements.fleetList.innerHTML = boats.map(boat => {
        ...
        return `<tr class="${boat.id === state.selectedId ? "selected" : ""}" data-boat-id="${boat.id}"><td><button class="dock-action ${action === "check-in" ? "check-in" : ""}" data-action="${action}" data-boat-id="${boat.id}" type="button" aria-label="${label} ${escapeHtml(boat.name)}" ${rented && !manualRental ? "disabled" : ""}>${label}</button></td><td><span class="boat-name">${escapeHtml(boat.name.replace("Rowboat ", ""))}</span><span class="boat-phase ${boat.late ? "late" : ""}">${boat.late ? "Running late" : PHASES[boat.phase].label}</span></td><td><span class="boat-status ${status.toLowerCase()}">${status}</span></td><td>${boat.guest ? escapeHtml(boat.guest.name) : "-"}</td><td>${boat.guest ? boat.guest.passengers : "-"}</td></tr>`;
    }).join("");
