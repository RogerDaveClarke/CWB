...
const state = { boats: new Map(), histories: new Map(), markers: new Map(), selectedId: null, routeLayer: null, historyUnsubscribe: null, search: "", status: "all", demoMode: false };
const elements = {
...
function renderTable() {
    ...
    if (!boats.length) { elements.tableBody.innerHTML = `<tr><td colspan="12" class="empty-cell">No vessels match the current filters.</td></tr>`; return; }
...
function subscribeToHistory(id) { if (state.historyUnsubscribe) state.historyUnsubscribe(); const app = window.__firebaseApp; if (!app) return; const historyQuery = query(collection(getFirestore(app), "boats", id, "history"), orderBy("timestamp", "desc"), limit(120)); state.historyUnsubscribe = onSnapshot(historyQuery, snapshot => { state.histories.set(id, snapshot.docs.map(document => { const data = document.data(); return { latitude: Number(data.latitude), longitude: Number(data.longitude), timestamp: toDate(data.timestamp) }; }).reverse()); drawRoute(id); renderTable(); renderMapDetail(); }, error => console.warn("Unable to load route history", error)); }
...
function startFirebase() {
    ...
    try {
        ...
        onSnapshot(collection(db, "boats"), snapshot => { const next = new Map(); snapshot.forEach(document => { const boat = normalizedBoat(document.id, document.data()); next.set(boat.id, boat); addLivePoint(boat); }); state.boats = next; if (!state.selectedId && next.size) state.selectedId = next.keys().next().value; setConnection("live", "Firebase live"); render(); }, error => { console.error("Firestore listener failed", error); setConnection("error", "Connection error"); elements.tableBody.innerHTML = `<tr><td colspan="12" class="empty-cell">Unable to load Firebase telemetry. Check the console and Firestore configuration.</td></tr>`; });
